...
"""Functional tests for nvidia/diar_streaming_sortformer_4spk-v2."""
...
NEMO_FILE = "nvidia__diar_streaming_sortformer_4spk-v2.nemo"
...
def test_model_inference():
    ...
    sr = 16000
    num_samples = 4 * sr  # 4 seconds
    n_spk = model._cfg.max_num_of_spks  # 4
    ...
    # preds: (batch, diar_frame_count, num_speakers) of sigmoid probabilities
    assert preds.ndim == 3, f"Expected 3-D output, got shape {preds.shape}"
    assert preds.shape[0] == 1, f"Expected batch size 1, got {preds.shape[0]}"
    assert preds.shape[2] == n_spk, f"Expected {n_spk} speakers, got {preds.shape[2]}"
    assert (
        preds.min() >= 0.0 and preds.max() <= 1.0
    ), f"Sigmoid outputs must be in [0, 1]; got [{preds.min():.4f}, {preds.max():.4f}]"
