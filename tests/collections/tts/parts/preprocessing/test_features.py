...
class TestTTSFeatures:
    ...
    def test_compute_energy(self):
        input_spec = [
            ...
            [
                [3.0, 0.0, 1.0], [0.5, 0.5, 2.0]
            ]
        ]
        input_spec = np.array(input_spec).transpose((0, 2, 1))
        expected_energy = [
            [0.0, np.sqrt(5.25)],
            [np.sqrt(10.0), np.sqrt(4.5)]
        ]
    ...
    def test_compute_mel_spectrogram(self):
        ...
        featurizer = AudioFeaturizer(mel_dim=spec_dim, hop_length=hop_len)
        spec = featurizer.compute_mel_spectrogram(input_audio)
    ...
    def test_compute_pitch(self):
        ...
        featurizer = AudioFeaturizer(hop_length=hop_len)
        pitch, voiced, voiced_prob = featurizer.compute_pitch(input_audio)

        assert len(pitch.shape) == 1
        assert pitch.shape[0] == expected_spec_len
        assert pitch.dtype == np.float
        ...
        assert voiced_prob.dtype == np.float