...
def compute_energy(spec: np.ndarray) -> np.ndarray:
    """
    Computes energy of the input spectrogram
...
class AudioFeaturizer():
    ...
    def read_audio(self, audio_path: Path) -> np.ndarray:
        """
        Load audio file with featurizer sample rate.

        Args:
            audio_path: Path to audio file.

        Returns:
            [T_audio] float array containing audio samples.
        """
    ...
    def compute_pitch(self, audio: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Computes pitch of the input audio

        Args:
            audio: [T_audio] float array containing audio samples.

        Returns:
            pitch: [T_spec] float array containing pitch for each spectrogram frame.
            voiced: [T_spec] bool array indicating whether each spectrogram frame contains speech.
            voiced_prob: [T_spec] float array with [0, 1] probability that each spectrogram frame contains speech.
        """
